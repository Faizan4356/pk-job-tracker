...
Phase 4 (deadline alerts) has been removed for now — it was built on Gmail/
SMTP and that's been dropped. Until a replacement alert channel is chosen,
use the dashboard (dashboard.py) to check upcoming deadlines. `pdf_extract.py`
is a separate, manually-run, opt-in step that backfills eligibility data
...
and was a deliberate one-off decision, not a default.
